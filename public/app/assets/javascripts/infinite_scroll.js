...
(function (exports) {
  ...
  InfiniteScroll.prototype.scrollBy = function (px) {
    ...
    if (
      px > 0 &&
      self.elt.find('.waypoint:last')[0].getBoundingClientRect().bottom -
        containerTop <
        self.wrapper.height()
    ) {
    ...
    self.elt[0].style.transform =
      'translateY(' + self.scrollPosition + 'px' + ')';
  ...
  InfiniteScroll.prototype.initScrollbar = function () {
    ...
    self.scrollbarElt.append(
      $('<div class="infinite-record-spacer" />').height(10000000)
    );
    self.scrollbarElt.height(self.wrapper.height());

    self.scrollbarElt.css('top', self.wrapper.offset().top);
    self.scrollbarElt.css(
      'left',
      self.wrapper.offset().left + self.wrapper.width() + 5
    );
    ...
    self.scrollbarElt.on('scroll', function (e) {
      ...
      self.scrollDragDelayTimer = setTimeout(function () {
        var targetRecord = Math.floor(
          (self.scrollbarElt.scrollTop() /
            self.scrollbarElt.find('.infinite-record-spacer').height()) *
            self.recordCount
        );
  ...
  InfiniteScroll.prototype.updateScrollPosition = function () {
    ...
    var pxOffset =
      (recordNumber / self.recordCount) * $('.infinite-record-spacer').height();
  ...
  InfiniteScroll.prototype.considerPopulatingWaypoints = function (
    preserveScroll,
    reentrant,
    done_callback
  ) {
  ...
  InfiniteScroll.prototype.populateWaypoints = function (
    waypointElts,
    preserveScroll,
    done_callback
  ) {
